...
def main():
    ...
    # auto generate and sync master_addr
    if args.master_addr == "auto":
        if args.nnodes <= 1:
            raise NotImplementedError()

        if args.node_rank == 0:
            args.master_addr = _get_local_ip()
            with open(".dist_master_addr", "w") as f:
                f.writelines([args.master_addr])
            print("Generate master address {}.".format(args.master_addr))
        else:
            for _ in range(600):
                if os.path.exists(".dist_master_addr"):
                    break
                time.sleep(1)
            with open(".dist_master_addr", "r") as f:
                args.master_addr = f.readline()
            print("Loaded master address {}.".format(args.master_addr))
